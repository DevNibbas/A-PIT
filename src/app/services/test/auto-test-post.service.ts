...
export class AutoTestPostService {
  ...
  testPost(url: string, paramNames: string[], paramRegex: string[], options: any): string[] {
    this.ret = [];
    const passVals = [this.apiContract.HTTP_POST_SUCCESS_CODE, this.apiContract.HTTP_POST_CREATED_CODE];
    const failVals =
      [this.apiContract.HTTP_POST_NO_CONTENT_CODE, this.apiContract.HTTP_POST_NOT_FOUND_CODE, this.apiContract.HTTP_POST_BAD_REQUEST];
  ...
  getAllReqsWH(): Observable<any>[] {
    const defReqWHeader = this.requestService.post(this.url, this.genData(-1), this.httpOptions);
    const ret = [defReqWHeader];
