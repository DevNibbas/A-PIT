...
export class AutoTestGetService {
  ...
  testGet(url: string, options: any): string[] {
    const passVals = [this.contractClass.HTTP_GET_SUCCESS_CODE];
    const failVals = [this.contractClass.HTTP_GET_NOT_FOUND_CODE, this.contractClass.HTTP_GET_BAD_REQUEST];
  ...
  getAllReqs(): Observable<any>[] {
    const woOptions: any = {};
    for (const i in this.httpOptions) {
      if (i !== 'headers') {
        woOptions[i] = this.httpOptions[i];
      }
    }
    const woHeader = this.requestService.get(this.url, woOptions);
    const wHeader = this.requestService.get(this.url, this.httpOptions);
